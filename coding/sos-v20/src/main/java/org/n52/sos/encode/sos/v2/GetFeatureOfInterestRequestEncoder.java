...
public class GetFeatureOfInterestRequestEncoder extends AbstractSosRequestEncoder<GetFeatureOfInterestRequest> {
    ...
    protected XmlObject create(GetFeatureOfInterestRequest request) throws OwsExceptionReport {
        GetFeatureOfInterestDocument doc = GetFeatureOfInterestDocument.Factory.newInstance(XmlOptionsHelper.getInstance().getXmlOptions());
        GetFeatureOfInterestType gfoit = doc.addNewGetFeatureOfInterest();
        gfoit.setService(request.getService());
        gfoit.setVersion(request.getVersion());
