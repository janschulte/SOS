...
public class GetObservationRequestEncoder extends AbstractSosRequestEncoder<GetObservationRequest> {
    ...
    protected XmlObject create(GetObservationRequest request) throws OwsExceptionReport {
        GetObservationDocument doc = GetObservationDocument.Factory.newInstance(XmlOptionsHelper.getInstance().getXmlOptions());
        GetObservationType got = doc.addNewGetObservation();
        got.setService(request.getService());
        got.setVersion(request.getVersion());
